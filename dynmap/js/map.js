...
DynMap.prototype = {
    ...
    configure: function (configuration) {
        ...
        $.each(me.options.worlds, function (index, worldentry) {
            ...
            $.each(worldentry.maps, function (index, mapentry) {
                ...
                if (me.options.defaultmap && me.options.defaultmap == mapentry.name) world.defaultmap = map;
    ...
    initialize: function () {
        ...
        if (typeof me.options.defaultzoom == "undefined") me.options.defaultzoom = 1;
                    ...
                    .append(
                        $("<a/>")
                            ...
                            .css({
                                backgroundImage:
                                    "url(" +
                                    (map.options.icon || "images/block_" + worldName + "_" + mapName + ".png") +
                                    ")",
                            })
        ...
        var alertbox = (me.alertbox = $("<div/>").addClass("alertbox").hide().appendTo(container));

        if (dynmapversion != me.options.coreversion && dynmapversion.indexOf("-Dev") < 0) {
    ...
    updateSidebarHeight: function () {
        ...
        if (sectionsHeight > sidebarHeight && sidebarHeight > minSectionsHeight) {
            ...
            $.each(me.sidebarSections, function (i, section) {
                ...
                var proportionalMissingHeight = missingHeight * (contentHeight / resizeableContentHeight);
    ...
    selectMapAndPan: function (map, location, completed) {
        ...
        compass.addClass("compass_" + map.options.compassview).addClass("compass_" + map.options.name);
        ...
        if (me.maptype.options.maxZoom < prevzoom) prevzoom = me.maptype.options.maxZoom;
        ...
        if (projectionChanged || worldChanged || location) {
            ...
            if (location) {
                ...
                var prevloc = null;
                if (prevmap != null) prevloc = prevmap.getProjection().fromLatLngToLocation(me.map.getCenter(), 64);
                if (prevloc != null) centerPoint = me.getProjection().fromLocationToLatLng(prevloc);
    ...
    update: function () {
        ...
        $.getJSON(
            me.formatUrl("update", { world: me.world.name, timestamp: me.lasttimestamp, reqid: me.reqid }),
            function (update) {
                ...
                me.playerfield.text(
                    me.options["msg-players"] + " [" + update.currentcount + "/" + me.options.maxcount + "]",
                );
            ...
            function (status, statusText, request) {
                ...
                if (me.missedupdates > 2) {
                    me.alertbox.text("Could not update map: " + (statusText || "Could not connect to server")).show();
    ...
    onTileUpdated: function (tileName, timestamp) {
        ...
        var a_b = true;
        if (prev && prev.indexOf("upd=0") > 0) a_b = false;
        var url = me.options.url.tiles;
        if (a_b) {
            if (url.indexOf("?") > 0) {
                this.registeredTiles[tileName] =
                    url + escape(me.world.name + "/" + tileName) + "&upd=0" + Math.random();
            } else {
                this.registeredTiles[tileName] =
                    url + escape(me.world.name + "/" + tileName) + "?upd=0" + Math.random();
            }
        } else this.registeredTiles[tileName] = url + me.world.name + "/" + tileName;
    ...
    updatePlayer: function (player, update) {
        ...
        // Update menuitem.
        if (me.options.grayplayerswhenhidden) player.menuitem.toggleClass("otherworld", me.world !== location.world);
    ...
    updateBackground: function () {
        ...
        var col = "#000000";
        if (me.serverday) {
            if (me.maptype.options.backgroundday) col = me.maptype.options.backgroundday;
            else if (me.maptype.options.background) col = me.maptype.options.background;
        } else {
            if (me.maptype.options.backgroundnight) col = me.maptype.options.backgroundnight;
            else if (me.maptype.options.background) col = me.maptype.options.background;
        }
    ...
    addToLayerSelector: function (layer, name, priority) {
        ...
        if (me.options.showlayercontrol != "false" && !me.layercontrol) {
            ...
            if (me.options.showlayercontrol == "pinned") me.layercontrol.options.collapsed = false;
    ...
    removeFromLayerSelector: function (layer) {
        ...
        for (i = 0; i < me.layersetlist.length; i++) {
            if (me.layersetlist[i].layer === layer) {
                ...
                if (me.options.showlayercontrol != "false") me.layercontrol.removeLayer(layer);
                break;
            }
        }
    },
